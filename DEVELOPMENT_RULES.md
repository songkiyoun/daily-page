...
- v0.7.3은 기초 전투 밸런스 마감판입니다.
- 단검의 대창 피해 보정 외에는 전투 구조를 변경하지 않습니다.
- 이후 밸런스 변화는 v0.7.3 스킬 시스템, 무기 진화, 고유 능력 추가 이후 다시 판단합니다.


## v0.7.3 개발 원칙

- v0.7.3은 스킬 완성판이 아니라 스킬 시스템 뼈대 버전입니다.
- 기본 스킬은 무기와 성격 정체성을 보여주는 약한 자동 발동 능력으로 시작합니다.
- 스킬 밸런스는 v0.7.3 이후 실제 전투와 50회 양방향 시뮬레이션으로 조정합니다.
- v0.7.3에서는 스킬 구조, 보상 구조, 레벨 구조가 정상 작동하는지 우선 확인합니다.
...
- v0.7.3 이후에는 각 스킬의 체감 강도를 실제 전투 기준으로 다시 조정합니다.
