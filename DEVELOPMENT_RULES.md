...
- 스턴은 한 번 걸리면 위험해야 하지만, 바로 다시 스턴이 반복되어 캐릭터가 계속 굳어 있으면 안 됩니다.
- 자세 붕괴 직후에는 짧은 보호 구간과 회복 보정을 주어 재붕괴 체인을 막습니다.
- 창 방어형의 강점은 유지하되, 동양검과 서양검을 연속 자세 피해로 일방적으로 묶는 흐름은 줄입니다.
- 단검 방어형은 완벽한 암살각을 기다리는 성격이 아니므로, 근접 측면에서는 약한 견제 찌르기를 시도해야 합니다.
- 모든 변경은 기존 함수 직접 수정 원칙을 유지합니다.

## v0.6.12 기준 작업 메모

- 방어형은 오래 버티되, 일정 시간 공격이 없으면 약한 견제 공격을 시도한다.
- 교착 해소용 견제 공격은 일반 공격보다 피해와 자세 피해가 낮다.
- 방어형을 강하게 만드는 것이 아니라, 전투가 멈춰 보이는 상황을 줄이는 것이 목적이다.

# DEVELOPMENT RULES

## v0.6.12 기준 작업 메모

- 단검·동양검 암살형의 측면 탐색은 무한 반복되면 안 됩니다.
- 한 방향 측면 탐색이 길어질 경우 반대 꺾기 또는 짧은 교전으로 전환해야 합니다.
- 단검은 공격 거리 근처에 들어간 뒤에는 후퇴보다 공격 시도 또는 측면 재정렬을 우선합니다.
- 창과 서양검의 대단검 대응은 회전속도 상향이 아니라 정면 견제, 방어 복귀, 밀쳐내기로 유지합니다.
- 모든 변경은 기존 함수 직접 수정 원칙을 유지합니다.
...
- `simulator.js`: 전투 코어 반복 실행과 조합 통계 계산
- `utils.js`: 공통 수학 함수
...
- 시뮬레이터는 실제 전투 로직을 복사하지 않고 기존 `state.js`, `battle.js` 함수를 사용합니다.
- 시뮬레이터 결과가 이상하면 전투 코어를 먼저 의심하고, 별도 보정 패치를 시뮬레이터에만 추가하지 않습니다.
- 시뮬레이터는 밸런스 판단 보조 도구이며, 실제 수동 관전 테스트를 완전히 대체하지 않습니다.


## v0.6.9 기준 추가 원칙

- 단검이 측후방을 잡는 개성은 유지하되, 같은 방식의 측후방 연속 피격은 상대 반응을 유발한다.
- 창은 몸 회전으로 단검을 따라가지 않고, 창대 밀쳐내기로 거리를 다시 만든다.
- 서양검은 몸 회전으로 단검을 추적하지 않고, 방어 복귀와 정면 가드·패링으로 대응한다.
- 동양검은 빠른 압박을 유지하되 단검 수준의 무한 측면 추적은 하지 않는다.
